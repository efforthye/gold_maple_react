...
router.post('/getImg', (req, res) => {
  if (req.body.currUserName)
    db.User.findOne({ where: { userName: req.body.currUserName } }).then((data) => {
      if (data.profileImg) {
...
router.post('/clearCookie', (req, res) => {
  db.User.update(
    ...
    db.User.findOne({ where: { userName: req.body.changeName } }).then((data) => {
      res.clearCookie('login');
      res.send({
        message: '쿠키 삭제 완료',
...
router.post('/comment', (req, res) => {
  db.Comment.findAll({ where: { userName: req.body.currUser } }).then((data) => {
    res.send(data);
  });
});
